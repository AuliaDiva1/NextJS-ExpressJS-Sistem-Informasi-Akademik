...
export default function EntryNilai() {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-white p-4 rounded-lg shadow">
          <h3 className="font-semibold mb-3">Entry Nilai</h3>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <ol className="list-decimal pl-5 space-y-1 text-sm text-gray-700">
            <li>Pilih jenis nilai yang akan diinput</li>
            <li>Pilih kelas dan mata pelajaran</li>
            <li>Klik tombol <b>Entry Nilai</b></li>
            <li>Isi nilai pada tabel yang muncul di bawah</li>
          </ol>
        </div>
      </div>
          ...
          <Column
            ...
            body={(row) => (
              <InputText
                ...
                onChange={(e) => handleChange(row.id, "pengetahuan", e.target.value)}
                className="w-20"
              />
            )}
          />
          <Column header="Pengetahuan (Predikat)" body={(row) => getPredikat(row.pengetahuan)} />
          <Column
            header="Pengetahuan (Deskripsi)"
            body={(row) => (
              <InputText
                value={row.pengetahuanDesc}
                onChange={(e) => handleChange(row.id, "pengetahuanDesc", e.target.value)}
                className="w-40"
              />
            )}
          />
          ...
          <Column
            ...
            body={(row) => (
              <InputText
                ...
                onChange={(e) => handleChange(row.id, "keterampilan", e.target.value)}
                className="w-20"
              />
            )}
          />
          <Column header="Keterampilan (Predikat)" body={(row) => getPredikat(row.keterampilan)} />
          <Column
            header="Keterampilan (Deskripsi)"
            body={(row) => (
              <InputText
                value={row.keterampilanDesc}
                onChange={(e) => handleChange(row.id, "keterampilanDesc", e.target.value)}
                className="w-40"
              />
            )}
          />
